...
// но в TypeScript void и undefined — это разные вещи.
// Они различаются по контекстной типизации.
// А происходит это из-за особенностей работы самого JavaScript.
// Самый яркий пример — метод forEach().

// Метод forEach() не использует данные, возвращаемые переданным колбеком, который вызывается внутри.
// Возможно, логично было бы определить возврат как undefined, но посмотрим на пример:
...
() => {
  ...
  //   Если бы forEach() требовал от колбека возврат undefined,
...
() => {
  ...
  // Хотя f() возвращает true, переменная v имеет тип void.
  // Это означает, что мы не можем использовать ее для чего-либо,
  // кроме как для присваивания другой переменной с типом void.
};

// Существует единственная ситуация, когда указание void явно запрещает возврат из функции.
// Это определение функции вне контекста использования, когда ее тип указывается явно:
...
// Также void является оператором JavaScript, поэтому важно не запутаться с ним.
// Он вычисляет выражение, которое следует за ним и возвращает undefined:
...
() => {
  function forEach(
    ...
    numbers.forEach((item, index) => callback(item, index));
  }
