...
//
// Опциональные параметры в функциях используются во встроенных функциях JavaScript,
// когда параметр необязателен.
// Опциональные параметры задаются с помощью знака вопроса после имени переменной перед двоеточием.
...
// Разработчики пытаются применять эту логику к колбекам, но сталкиваются с ошибками.
...
() => {
  ...
  function filter(
    coll: number[],
    callback: (arg: number, index?: number) => boolean
  ) {
    const result: number[] = [];
  ...
  // Как и во встроенном методе filter(), в этой функции колбек принимает вторым параметром
  // индекс обрабатываемого элемента массива. Обычно его не используют,
  // но иногда фильтрация массива делается на основе индексов. И в этой ситуации его указывают.
  // Для этого мы пометили этот параметр как необязательный.
...
// В данном случае ошибка обозначает, что из-за необязательности внутри колбека
// в теории может оказаться undefined. Хотя по смыслу это происходить не может,
// так как индекс всегда определен.
...
// Если В JavaScript функция вызывается с большим количеством параметров, чем определено,
// то лишние игнорируются. TypeScript ведет себя так же.
// Колбеки с меньшим числом параметров всегда могут появляться там, где они же ожидаются
// с большим числом параметров, при условии, что типы общих параметров совпадают.

// В примере выше аргументом передается функция (n) => n > 1, а вызывается она как callback(n, index).
// Это приводит к игнорированию index, при этом никаких ошибок типов не возникает.
