...
// Так что если у animal много полезных свойств и методов,
// то они автоматически становятся доступными у rabbit.
...
// JavaScript будет искать его в rabbit, а затем в animal.
...
() => {
  ...
  // Теперь вызов rabbit.walk() находит метод непосредственно
...
// Это на самом деле очень важная деталь, потому что у нас может быть большой объект
// со множеством методов, от которого можно наследовать.
// Затем наследующие объекты могут вызывать его методы, но они будут изменять своё состояние,
// а не состояние объекта-родителя.
...
() => {
  ...
  // Если бы у нас были другие объекты, такие как bird, snake и т.д., унаследованные от animal,
  // они также получили бы доступ к методам animal. Но this при вызове каждого метода будет
  // соответствовать объекту (перед точкой), на котором происходит вызов, а не animal.
...
// Заметим ещё одну деталь. Откуда взялся метод rabbit.hasOwnProperty? Мы его явно не определяли.
...
// - Если мы вызываем obj.method(), а метод при этом взят из прототипа, то this всё равно ссылается
//   на obj. Таким образом, методы всегда работают с текущим объектом, даже если они наследуются.
