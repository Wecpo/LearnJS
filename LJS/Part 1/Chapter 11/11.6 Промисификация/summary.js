...
// На практике нам, скорее всего, понадобится промисифицировать не одну функцию, поэтому имеет смысл сделать
// для этого специальную «функцию-помощник».

// Мы назовём её promisify(f) – она принимает функцию для промисификации f и возвращает функцию-обёртку.

// Эта функция-обёртка делает то же самое, что и код выше: возвращает промис и передаёт вызов оригинальной f,
// отслеживая результат в своём колбэке:
...
() => {
  ...
  loadScriptPromise(someTest).then(someTest1);
};
...
// Для более экзотических форматов колбэка, например без err: callback(result), мы можем промисифицировать функции
// без помощника, «вручную».
